...
function FileSidebar(props:{localeStr: string, files:string[], userName: string}) {
    ...
    const items = props.files.map((file, index) => {
        return <FileSidebarItem name={file} id={index} key={index.toString()} click={selectFile} />
    });
    useEffect(() => {  

        var selectedIds = Object.keys(selectedFiles).filter(id => selectedFiles[id]);
          console.log(selectedIds);
          var newSelectedItems = selectedIds.map((id) => {
            return <FileSidebarSelectedItem name={props.files[parseInt(id)]} id={parseInt(id)} key={"selected " + id.toString()} click={removeFile} />
          });
