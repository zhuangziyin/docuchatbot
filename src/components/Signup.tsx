  ...
  return (<>
    <div className={styles.loginBox + " rounded-2xl"}>
      <div className={styles.loginLblBox}>
        ...
        <div className={"flex items-center justify-center text-xl " + styles.lineTopMargin + ' ' + fontHeavyCls}>{t("SignUp")}</div>
              <div className={"flex items-center justify-center text-sm " + styles.lineTopMargin + ' ' + fontCls}>{t("SignUpDesc")}</div>
      </div>
      ...
      {isError && <div className="self-center relative -top-2 text-red-500 text-sm bg-red-500/20 px-2 py-2 border border-red-300 rounded-md w-fit flex items-center">
        ...
        <div className={styles.loginContentLbl+ ' text-xs'}>{t("UserName")}</div>
        <input ref={usernameRef} className={"border-gray-300 py-2 px-3 rounded-md mt-1 " + styles.clearInput} onKeyDown={removeErrHint}  name="name"></input>

        <div className={styles.loginContentLbl+ ' text-xs'} style={{marginTop:'1rem'}}>{t("Email")}</div>
        <input ref={emailRef} className={"border-gray-300 py-2 px-3 rounded-md mt-1 " + styles.clearInput}  onKeyDown={removeErrHint} name="email"></input>
        ...
        <button className="rounded-md border-black text-sm text-white bg-black border-black  py-2 px-3 mt-4" onClick={handleSubmit}>{t("SignUp")}</button>
        <div className="flex items-center justify-center text-sm mt-3">
          <span>{t("HasAccountHint")}</span>&nbsp;<a href={t("SignInLink")!} className="text-bold ">{t("SignIn")}</a>
        </div>
      </div>
    </div>
  </>)
